...
// This is the length from front to CoG (obtained from Project walkthrough).
const double Lf = 2.67;

// Reference error values
double ref_cte = 0;
double ref_epsi = 0;
double ref_v = 70; //tries to maintain this speed

// Cost weights that determine how each term of the cost function is weighted
double cte_weight = 2000;
double epsi_weight = 2000;
double v_weight = 1;
double delta_weight = 500;
double a_weight = 50;
double delta_diff_weight = 2000;
double a_diff_weight = 10;

// Save starting indices for each variable, since all variables are
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // The rest of the constraints at the other timesteps
    for (int t = 1; t < N; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set initial variable values to initial state
  vars[x_start] = x;
  vars[y_start] = y;
  vars[psi_start] = psi;
  vars[v_start] = v;
